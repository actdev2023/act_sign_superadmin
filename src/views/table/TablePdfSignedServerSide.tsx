...
const TablePdfSignedServerSide: React.FC<FilesTableProps> = ({ onView }) => {
    ...
    const columns: GridColumns = [
        ...
        },
        {
          ...
          headerName: 'Actions',
          renderCell: (params: GridRenderCellParams) => (
            <ActionsColumn id={params.row.id} onDownload={handleDownload} />
          ),
        }
  ...
  const handleDownload = async (id: number) => {
    try {
            const response = await fetch(`${API_URL}/api/download-pdf/${id}`, {
                method: 'GET',
                headers: {
            ...
            if (response.ok) {
                ...
                console.error('Failed to download the signed PDF');
            }
        } catch (error) {
            console.error('Error downloading the signed PDF:', error);
        }
  }
...
export default TablePdfSignedServerSide